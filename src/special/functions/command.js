module.exports = {
    ...
    func: async function (matches, msg, isBot, _, opts) {
        ...
        if (command || localCommand) {
            var isDisabled = data.guildData[msg.guild.id].disabled.find(cmd => cmd.find(n => n === commandname)) && !bypassPerms
            ...
            if (isDisabled) {
                ...
                if (command) {
                    ...
                    await command.execute.call(poopy, msg, args, { ownermode: opts.ownermode, isBot, sourceMsg: opts.sourceMsg }).catch(err => {
                        error = err.stack
                    })
