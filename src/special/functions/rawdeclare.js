module.exports = {
    ...
    func: async function (matches, msg, isBot, string, opts) {
        ...
        tempdata[msg.author.id][msg.id].declared[`{${name}}`] = value.replace(new RegExp(`\\{${name}\\}`, 'ig'), tempdata[msg.author.id][msg.id].declared[`{${name}}`] || '')
        tempdata[msg.author.id][msg.id].keyDeclared[`{${name}}`] = {
            func: async function (msg, isBot, _, opts) {
                return await parseKeywords(value.replace(new RegExp(`\\{${name}\\}`, 'ig'), tempdata[msg.author.id][msg.id].declared[`{${name}}`] || ''), msg, isBot, opts).catch(() => { }) ?? ''
            },
            declared: true
        }
        return [phrase, true]
