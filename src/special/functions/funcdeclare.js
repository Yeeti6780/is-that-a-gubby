module.exports = {
    ...
    func: async function (matches, msg, isBot, string, opts) {
        ...
        tempdata[msg.author.id][msg.id].declared[`[${name}]`] = value
            ...
            func: async function (matches, msg, isBot, _, opts) {
                ...
                valOpts.extraFuncs.val = {
                    func: async function (matches, msg, isBot, _, opts) {
                        ...
                        if (word.endsWith('+')) return await parseKeywords(split.slice(index).join(' ') || '', msg, isBot, opts).catch(() => { }) || ''
                        return await parseKeywords(split[index] || '', msg, isBot, opts).catch(() => { }) || ''
                    }
                }

                return await parseKeywords(value.replace(new RegExp(`\\[${name}\\]\\(([\\s\\S]*?)\\)`, 'ig'), tempdata[msg.author.id][msg.id].declared[`[${name}]`] || ''), msg, isBot, valOpts).catch(() => { }) ?? ''
            },
            raw: true,
            declared: true
        }
