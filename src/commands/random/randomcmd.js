        ...
        function chooseCmd() {
            if (type) {
                ...
                for (var i in allCmds) {
                    var cmd = allCmds[i]
                    if (cmd.type === type && !(cmd.type === 'Owner' || cmd.type === 'JSON Gang' || cmd.type === 'Tumore' || cmd.perms || data.guildData[msg.guild.id].disabled.find(c => c.find(n => n === cmd.name.find(nn => nn === n))))) {
                        foundCmds.push(cmd)
                    }
                }
                ...
                if (cmd.type === 'Owner' || cmd.type === 'JSON Gang' || cmd.type === 'Tumore' || cmd.perms || data.guildData[msg.guild.id].disabled.find(c => c.find(n => n === cmd.name.find(nn => nn === n)))) {
                    return chooseCmd()
                }
                return cmd
            }
