module.exports = {
    ...
    execute: async function (msg, args) {
        ...
        if ((filetype.mime.startsWith('image') && !(vars.gifFormats.find(f => f === filetype.ext))) && (filetype2.mime.startsWith('image') && !(vars.gifFormats.find(f => f === filetype2.ext)))) {
            ...
            await execPromise(`ffmpeg -stream_loop -1 -r ${fps2} -i ${filepath}/${filename} -r ${fps2} -i ${filepath}/${filename2} -filter_complex "[0:v]scale=${width}:-1[file];[1:v]scale=${width}:-1[file2];[file][file2]vstack=shortest=1[qout];[qout]scale='min(1000,iw)':min'(400,ih)':force_original_aspect_ratio=decrease[rout];[rout]split[gnout][gpout];[gpout]palettegen=reserve_transparent=1[palette];[gnout][palette]paletteuse=alpha_threshold=128[out]" -map "[out]" -preset ${findpreset(args)} -gifflags -offsetting ${filepath}/output.gif`)
