module.exports = {
    ...
    execute: async function (msg, args) {
        ...
        if (type.mime.startsWith('image')) {
            ...
            await execPromise(`ffmpeg -stream_loop -1 -t ${duration} -i ${filepath}/${filename} -r 50 -stream_loop -1 -t ${duration} -i assets/image/transparent.png -filter_complex "[1:v][0:v]scale=w=rw+rw/10:h=rh[transparent];[0:v]fps=50,scale=iw+sin(PI/2*(t*4/${duration}))*(iw/10):ih+cos(PI/2*(t*4/${duration}))*(ih/3)-ih/3:eval=frame[ooverlay];[transparent][ooverlay]overlay=x=(W-w)/2:y=(H-h):format=auto,scale='min(1000,iw)':min'(400,ih)':force_original_aspect_ratio=decrease,split[pout][ppout];[ppout]palettegen=reserve_transparent=1[palette];[pout][palette]paletteuse=alpha_threshold=128[out]" -map "[out]" -aspect ${Math.floor(width + width / 10)}:${height} -preset ${findpreset(args)} -gifflags -offsetting -r 50 -t ${duration} ${filepath}/output.gif`)
